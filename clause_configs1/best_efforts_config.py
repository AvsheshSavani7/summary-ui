BEST_EFFORTS_CLAUSES = {
    ...
    "Best Efforts : Remedy Actions": {
        "question": "divestiture_commitments?",
        "conditions": [
            {
                "question": "What are the Divestiture Comitments of the Company",
                "if": "best_efforts.remedy_actions_other.remedy_definitions_provided.answer",
                "type": "boolean",
                "true": {
                    "add_to_prompt": {
                        "remedy_action_text1": "{{best_efforts.remedy_actions_other.remedy_definitions_provided.clause_text}}",
                        "remedy_action_answer": "{{best_efforts.remedy_actions_other.remedy_definition_text.answer}}",
                        "remedy_action_full_text": "{{best_efforts.remedy_actions_other.remedy_definition_text.clause_text}}"
                    }
                },
                "false": {
                    "text_output": ""
        ...
        "prompt_template": (
            "You are a legal and finance analyst reviewing a merger agreement.\n\n"
            "Task:\n"
            "- Focus specifically on limitations, exclusions, and thresholds related to **divestiture and other remedy actions** the buyer (Parent) and its subsidiaries may be required to take.\n"
            "- Ignore general best efforts or regulatory cooperation obligations unless they contain **explicit carveouts or caps** on remedy actions.\n"
            "- If the agreement contains one or more **carveouts**, such as:\n"
            "   (a) Parent not required to take actions affecting its own business, or\n"
            "   (b) a divestiture cap based on revenue or materiality for the target’s business,\n"
            "then summarize each limitation in its own concise sentence or paragraph.\n"
            "- Be specific: state the financial threshold (e.g., $140 million), the party it applies to, and the scope of the exclusion.\n"
            "- If no carveouts, exclusions, or caps are found, clearly say so.\n\n"
            "--- Clause Texts\n"
            "Remedy Action Clause Text: {remedy_action_text1}\n"
            "Remedy Action Clause Answer: {remedy_action_answer}\n"
            "Remedy Action Clause Full Text: {remedy_action_full_text}\n\n"
            "--- Example Outputs:\n"
            "Parent and its subsidiaries are not required to take any Remedy Action on their own businesses or products.\n\n"
            "There is a divestiture cap on the Company’s businesses that generated over $140 million in 2024 sales.\n\n"
            "No limitations apply to foreign antitrust or HSR approvals."
        ),

        "output_field": "remedy_action_summary",
        "reference_fields": ["best_efforts.remedy_actions_other.remedy_definitions_provided.reference_section"],
    ...
    "Best Efforts : Litigation Commitments": {
        ...
        "prompt_template": (
            "You are a legal analyst reviewing a merger agreement clause related to litigation requirements.\n\n"
            "Task:\n"
            "- Summarize the litigation obligation in **one sentence**.\n"
            "- Use the exact party name as stated if available.\n"
            "- Use this phrasing format: '[Party] is required to defend against any litigation.'\n"
            "- Do not paraphrase or add additional commentary.\n\n"
            "-- Clause Detail:\n"
            "{litigation_requirement_status}\n\n"
            "-- Example Outputs:\n"
            "- BMY is required to defend against any litigation.\n"
            "- Nippon is required to defend against any litigation.\n"
            "- JHX is required to defend against any litigation."
        ),
    ...
    "Best Efforts : Right to Control Strategy": {
        "question": "Right to Control Strategy",
        "conditions": [
            {
                "question": "Right to Control Strategy",
                "if": "best_efforts.regulatory_strategy_control_rights.regulatory_strategy_control_rights.answer",
                "type": "enum",
                ...
                "default": {
                    "add_to_prompt": {
                        "right_to_control_strategy": "{{best_efforts.regulatory_strategy_control_rights.regulatory_strategy_control_rights.clause_text}}"
                    }
                }
        ...
        "prompt_template": (
            "You are a legal analyst reviewing merger agreement terms related to regulatory strategy and timing control.\n\n"
            "Task:\n"
            "- Write one concise bullet point explaining which party has the right to control strategy or timing for obtaining antitrust or regulatory approvals.\n"
            "- Include key limitations, such as consent rights, restrictions on pulling/refiling, or control over timing agreements.\n"
            "- Use formal M&A language and preserve party names exactly as stated.\n"
            "- Do not generalize or omit critical constraints.\n\n"
            "-- Clause Detail:\n"
            "{right_to_control_strategy}\n\n"
            "-- Example Outputs:\n"
            "- BMY shall have the right to direct strategy for obtaining antitrust approvals and controlling the defense of any litigation brought pursuant to antitrust laws.\n"
            "- Nippon may not pull and refile or enter into any timing agreement with respect to any regulatory reviews without X’s consent, but Nippon shall control strategy for obtaining the Required Approvals."
        ),
        "output_field": "right_to_control_strategy",
        "reference_fields": ["best_efforts.regulatory_strategy_control_rights.regulatory_strategy_control_rights.reference_section"],
        "use_short_reference": True,
        "summary_type": "Concise",
        "format_style": "paragraph",
        "max_words": 50,
    ...
    "Best Efforts : FTC Warning Letter Handling Concise": {
        ...
        "prompt_template": (
            ...
            "Write one concise sentence explaining how the agreement addresses the receipt of an FTC warning letter.\n"
    ...
    "Best Efforts : Union Handling": {
        "question": "Union Handling",
        "conditions": [
            {
                "question": "Union Handling",
                "if": "best_efforts.union_item.union_efforts_required.answer",
                "type": "boolean",
                "true": {
                    "add_to_prompt": {
                        "union_efforts": "{{best_efforts.union_item.union_efforts_required.clause_text}}",
                        "union_efforts_timing": "{{best_efforts.union_item.union_efforts_required.clause_text}}"
                    }
                },
                "false": {
                    "text_output": ""
        ...
        "prompt_template": (
            "You are a legal analyst reviewing a merger agreement for labor-related provisions.\n\n"
            "Task:\n"
            "- Write one concise bullet point explaining how the agreement addresses union-related efforts and timing.\n"
            "- Focus on requirements involving organized labor, union negotiations, or obligations to consult or coordinate with unions (e.g., the USW).\n"
            "- Identify any cooperation obligations, timing requirements, or efforts standards (e.g., reasonable best efforts, good faith).\n"
            "- Use formal M&A language and retain party names as written.\n"
            "- Do not paraphrase or generalize beyond the provided text.\n\n"
            "-- Union Efforts Clause:\n"
            "{union_efforts}\n\n"
            "-- Union Efforts Timing Clause:\n"
            "{union_efforts_timing}\n\n"
            "--- Example Output:\n"
            "- X shall cooperate with Nippon and use reasonable best efforts to facilitate meetings between Nippon and the USW for purposes of assuming the USW Agreements."
        ),
        "output_field": "union_efforts",
        "reference_fields": ["best_efforts.union_item.union_efforts_required.reference_section"],
        "use_short_reference": True,
        "summary_type": "Concise",
        "format_style": "paragraph",
        "max_words": 40,
    ...
    "Best Effort : Timing agreement": {
        ...
        "output_field": "timing_controls_summary",
    ...
    "Best Effort : FTC Warning Letter Handling Fulsome": {
        "question": "ftc_warning_letter_handling",
        "conditions": [
            {
                "question": "ftc_warning_letter_handling",
