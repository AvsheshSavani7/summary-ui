...
def evaluate_condition_branch(condition, data):
    if "conditions" in condition:
        ...
        if run_if == "all":
            should_run = all("triggered" in res for res in subresults)
        elif run_if == "any":
            should_run = any("triggered" in res for res in subresults)
        else:
            should_run = False
    ...
    if condition["type"] == "boolean":
        ...
        output = {}  # ✅ Make sure output is defined at the top
        value_normalized = (value or "").strip()
        enum_cases = condition.get("enum_cases", {})
        
        if value_normalized in enum_cases:
            print(f"✅ Matched enum case: {value_normalized}")
            branch = enum_cases[value_normalized]
        else:
            print(f"❌ No enum match for: {value_normalized}")
        ...
        if "add_to_prompt" in branch:
            ...
            for k, v in branch["add_to_prompt"].items():
                if isinstance(v, str) and "." in v:
                    resolved_val = get_nested_value(data, v)
                    resolved_prompt_fields[k] = resolved_val if resolved_val is not None else v
                else:
                    resolved_prompt_fields[k] = v
    ...
    if "add_to_prompt" in branch:
        resolved_prompt_fields = {}
        for k, v in branch["add_to_prompt"].items():
            # Attempt to resolve v as a data path
            if isinstance(v, str) and "." in v:
                resolved_val = get_nested_value(data, v)
                if resolved_val is not None:
                    resolved_prompt_fields[k] = resolved_val
                else:
                    resolved_prompt_fields[k] = v  # fallback to literal if not found
            else:
                resolved_prompt_fields[k] = v
        output.setdefault("add_to_prompt", {}).update(resolved_prompt_fields)
    if "add_references" in branch:
